set_property PACKAGE_PIN W5 [get_ports {clk}]
set_property IOSTANDARD LVCMOS33 [get_ports clk]

set_property PACKAGE_PIN R2 [get_ports {prst}]
set_property IOSTANDARD LVCMOS33 [get_ports prst]

set_property PACKAGE_PIN V17 [get_ports {flush_flash}]
set_property IOSTANDARD LVCMOS33 [get_ports flush_flash]

set_property PACKAGE_PIN V3 [get_ports {eq_flash}]
set_property IOSTANDARD LVCMOS33 [get_ports eq_flash]

set_property PACKAGE_PIN W18 [get_ports {ee}]
set_property IOSTANDARD LVCMOS33 [get_ports ee]

#buttons
set_property PACKAGE_PIN U18 [get_ports {C}]
set_property IOSTANDARD LVCMOS33 [get_ports C]

set_property PACKAGE_PIN T18 [get_ports {U}] 
set_property IOSTANDARD LVCMOS33 [get_ports U]

set_property PACKAGE_PIN T17 [get_ports {R}] 
set_property IOSTANDARD LVCMOS33 [get_ports R]

set_property PACKAGE_PIN U17 [get_ports {D}]
set_property IOSTANDARD LVCMOS33 [get_ports D]

set_property PACKAGE_PIN W19 [get_ports {L}] 
set_property IOSTANDARD LVCMOS33 [get_ports L]

#leds
set_property PACKAGE_PIN L1 [get_ports {dis_mode[3]}] 
set_property IOSTANDARD LVCMOS33 [get_ports dis_mode[3]]

set_property PACKAGE_PIN P1 [get_ports {dis_mode[2]}] 
set_property IOSTANDARD LVCMOS33 [get_ports dis_mode[2]]

set_property PACKAGE_PIN N3 [get_ports {dis_mode[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports dis_mode[1]]

set_property PACKAGE_PIN P3 [get_ports {dis_mode[0]}] 
set_property IOSTANDARD LVCMOS33 [get_ports dis_mode[0]]

#set_property PACKAGE_PIN E19 [get_ports {pC}] 
#set_property IOSTANDARD LVCMOS33 [get_ports pC]

#set_property PACKAGE_PIN U19 [get_ports {pL}] 
#set_property IOSTANDARD LVCMOS33 [get_ports pL]

#set_property PACKAGE_PIN U16 [get_ports {pR}] 
#set_property IOSTANDARD LVCMOS33 [get_ports pR]


#7seg
set_property PACKAGE_PIN W7 [get_ports {O[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {O[6]}] 

set_property PACKAGE_PIN W6 [get_ports {O[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {O[5]}] 

set_property PACKAGE_PIN U8 [get_ports {O[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {O[4]}]

 set_property PACKAGE_PIN V8 [get_ports {O[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {O[3]}]

set_property PACKAGE_PIN U5 [get_ports {O[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {O[2]}]

set_property PACKAGE_PIN V5 [get_ports {O[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {O[1]}]

set_property PACKAGE_PIN U7 [get_ports {O[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {O[0]}]

# Anodes
set_property PACKAGE_PIN W4 [get_ports {anode_active[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {anode_active[3]}]

set_property PACKAGE_PIN V4 [get_ports {anode_active[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {anode_active[2]}]

set_property PACKAGE_PIN U4 [get_ports {anode_active[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {anode_active[1]}]

set_property PACKAGE_PIN U2 [get_ports {anode_active[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {anode_active[0]}]